...
const LateComingPage = () => {
    ...
    const fetchLateRecords = async () => {
        ...
        try {
            ...
            const lateRecords = res.data.filter(rec => rec.status === 'Late');
    ...
    const handleExport = () => {
        ...
        const headers = user.role !== 'Employee'
            ? ['Employee Name', 'Employee ID', 'Date', 'Check In', 'Status']
        ...
        link.setAttribute('download', `Late_Coming_Report_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-[var(--text-primary)]">Late Coming Records</h1>
                    <p className="text-[var(--text-secondary)] mt-1">
                        {user.role === 'Employee' ? 'View your late coming instances.' : 'Monitor and manage late coming records.'}
                    </p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-[var(--border-color)]">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={user.role !== 'Employee' ? 4 : 3} className="px-6 py-12 text-center text-slate-500 italic">No late coming records found.</td>
                                </tr>
                            ) : (
                                records.map((rec) => (
                                    <tr key={rec.id} className="hover:bg-slate-800/20 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider">
                                                {rec.status}
                                            </span>
                                        </td>
                                    </tr>
